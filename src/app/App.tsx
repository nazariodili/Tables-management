...
function SidebarToggle({ label, side, open, onToggle, variant }: {
  ...
  const icon = <Icon size={variant === "floating" ? 16 : 17} className="shrink-0" />;
  const text = (
    <span className={variant === "header" ? "text-base font-semibold" : "text-sm font-semibold"}>
      {label}
    </span>
  );
  ...
  return (
    <button onClick={onToggle} title={open ? `Hide ${label}` : `Show ${label}`}
      className={variant === "floating"
        ...
        : "flex items-center gap-1.5 text-gray-500 hover:text-gray-800 transition-colors min-w-0"}>
...
export default function App() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setSelectedShapeId(null); setSelectedTableId(null); setEditingShapeId(null); setPageMenu(null); return; }
